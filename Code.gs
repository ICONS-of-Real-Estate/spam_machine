...
const CONFIG = {
  ...
  CODE_VERSION: 'f1629f3 (30 Aug 2026)',
      ...
      if (!isAliasItself) {
        ...
        if (!forwardInfo) {
          ...
          if (plainBody.trim() === '') {
            ...
            // DIAGNOSTIC (27 Aug 2026, same request -- Wendy's thread): her
            // body is ENTIRELY a quote of Joana's own reply, with no fresh
            // text and no deeper nested "On ... wrote:" line from Wendy
            // herself visible even at 800 chars. That shape only reaches
            // this function because isAliasItself was true for the last
            // message -- i.e. lastMsg's own sender is the network alias or
            // an internal/sending alias, not Wendy. If lastMsg is really just
            // Joana's own sent reply reflected back into the thread (e.g. by
            // the network mailing-list mechanism), its own envelope To: is
            // very likely Wendy's real address -- but that's a guess, not a
            // confirmed shape like the header-block cases above, so logging
